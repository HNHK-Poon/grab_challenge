...
parser.add_argument('--features', metavar='F', default=['acceleration_x','acceleration_y','acceleration_z','gyro_x','gyro_y'], type=list,
...
for df in sample_dfs:
    for index, F in enumerate(FEATURES):
        ...
        df_id = normalize_specific(df_id,'total_out',(0,1))
        ...
        plt.show()
        df_id[-155:].to_csv(F+'result.csv')
    df_acc = normalize_specific(df_acc[-155:],'acc',(0,1))    
    df_acc['second'] =  df_acc['second']-310
    #ax = sns.lineplot(x="second", y=F, data=df_acc)
